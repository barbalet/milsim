...
The current build is a playable tactical slice with completed Cycle 2 environment systems, persistence foundations, a broader Cycle 3 combat pass, a deeper `First-Person Presentation` cycle, and an expanding Cycle 4 command layer: run four mission seeds, move through terrain tiles and compounds, use stance and lean to manage exposure, switch fire modes, vault low cover, collect military gear, work gates/crates/radios/emplaced guns, recover objective packages, and extract while a three-operator fireteam follows, holds, or assaults on command. Weapons, pickups, mission loot placement, and mission-script text load from bundled JSON, while the tactical map tracks discovered markers, live radio reports, authored mission phases, outcome branches, friendly positions, enemy alert/search states, and a command route through the battlefield. The SwiftUI shell can also save and reload live campaign archives from Alpha, Bravo, or Charlie slots via the HUD or menu bar, score completed operations across stealth, tempo, casualties, and recovered materiel, and surface a mission debrief after success or failure. Firefights now show head and limb wounds, fractures, stamina shock, splint treatment, enemy fallback reactions, search behavior off the last known contact, broader penetration behavior through select cover, vegetation damping, stronger weapon-signature feedback, and a toggleable first-person view with a depth-tested 3D world pass, terrain-aware skyline layers, richer terrain and prop breakup, warmer/cooler atmospheric lighting, more cohesive field palettes for terrain and props, world-hit sight and laser alignment, depth-aware focus prompts, muzzle flashes, material-specific impact bursts, smoother viewmodel transitions, reload and melee presentation beats, spring-driven recoil recovery, and visible friendly operators over the same tactical simulation.
...
- `T`: cycle fireteam order (`Follow`, `Hold`, `Assault`)
- `B`: toggle fire mode
- `C`: crouch toggle
- `Z`: prone toggle
- `Q` / `E`: lean left / right
...
- Lightweight fireteam command layer with three AI teammates, cycleable `Follow` / `Hold` / `Assault` orders, and friendly tactical-map tracking
- Mission scoring and debrief presentation built around stealth, tempo, casualties, and recovered materiel, with best-score tracking stored in campaign progress
- Enemy awareness reporting with patrol/alert/search/engaged/fallback state changes driving tactical-map labels, route pressure summaries, radio chatter, and debrief stealth pressure
